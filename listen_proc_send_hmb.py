...
def shellprocess_manager_singlethread(hmb,**pubopt):
    process_queue = Queue()
    hmbthread = Process(name='hmbthread', target=launch_hmb, args=(process_queue, hmb))
    hmbthread.start()
...
if __name__ == '__main__':
    ...
    url = args.url
    pubopt = {'agency':'SED-ETHZ',
              'url':'http://cerf.emsc-csem.org/ETH',
              'user':user,
              'password':password,
              'queue_pub':'FINDER', 
              }
    ...
    if args.nothread:
        ...
        shellprocess_manager_singlethread(hmb,**pubopt)
    else:
